    ...
    <div class="card overflow-hidden">
        <div class="table-responsive overflow-x-auto">
            <table>
                ...
                <tbody>
                    @forelse($comments as $index => $c)
                    <tr>
                        <td>{{ $comments->firstItem() + $index }}</td>
                        <td>
                            <img src="{{ $c->avatar_url }}" alt="Avatar" class="w-8 h-8 rounded-full object-cover border border-slate-200">
                        </td>
                        <td>
                        ...
                        <td style="text-align: center;">
                            <div class="inline-flex items-center gap-1">
                                ...
                                <form action="{{ route('admin.comments.toggle', $c->id) }}" method="POST">
                                    @csrf
                                    @method('PATCH')
                                ...
                                {{-- Hapus Komentar --}}
                                <form action="{{ route('admin.comments.destroy', $c->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus komentar ini?')">
                                    @csrf
                                    @method('DELETE')
