{{-- resources/views/admin/dashboard/announcement.blade.php --}}
<div class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden flex flex-col h-[520px]">
    ...
    <div class="px-5 py-4 border-b border-slate-100 dark:border-slate-700/60 bg-white dark:bg-slate-800 sticky top-0 z-10 flex items-center justify-between">
        ...
        <a href="{{ route('admin.pengumuman') }}" class="text-[10px] font-black text-indigo-600 uppercase hover:underline">Kelola</a>
        @endif
    </div>

    <div class="flex-1 overflow-y-auto custom-scrollbar">
        @forelse($widgetPengumuman as $item)
            @php
                $wData = [
            ...
            @endphp
            <button onclick='wdgBuka(@json($wData))' class="w-full text-left p-4 border-b border-slate-50 dark:border-slate-700/50 hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-all group">
                <div class="flex gap-4">
                    ...
                    <div class="w-20 h-20 rounded-2xl overflow-hidden border border-slate-100 dark:border-slate-600 shrink-0 bg-slate-100">
                        <img src="{{ asset('storage/'.$item->file_path) }}" class="w-full h-full object-cover group-hover:scale-110 transition-transform">
                    </div>
                    @else
                    <div class="w-14 h-14 rounded-2xl {{ $ikBg }} dark:bg-slate-700 flex items-center justify-center text-2xl shrink-0 border border-current opacity-60">
                        {{ $item->tipeIcon() }}
                    </div>
                    @endif
                    <div class="flex-1 min-w-0">
                        <div class="flex items-center gap-2 mb-1.5 flex-wrap">
                            <span class="text-[9px] font-black uppercase tracking-widest px-2 py-0.5 rounded-md {{ $item->audienceBadgeColor() }}">
                                {{ $item->audienceLabel() }}
                            </span>
                            <span class="text-[9px] font-bold text-slate-400 uppercase">{{ $item->created_at->diffForHumans() }}</span>
                        </div>
                        <h4 class="text-xs font-black text-slate-800 dark:text-white leading-tight group-hover:text-indigo-600 transition-colors line-clamp-2 mb-2">{{ $item->judul }}</h4>
                        <p class="text-[10px] text-slate-400 leading-relaxed line-clamp-2">{{ strip_tags($item->isi) }}</p>
                    </div>
                </div>
            </button>
        @empty
...
{{-- ============================================================
     MODAL OVERLAY DETAIL PENGUMUMAN
     Letakkan di luar div widget agar tidak terpotong overflow-hidden
     ============================================================ --}}
<div id="wdgModal"
