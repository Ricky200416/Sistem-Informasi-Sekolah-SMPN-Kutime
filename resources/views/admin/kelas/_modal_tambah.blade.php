            ...
            <form id="formTambahKelas" action="{{ route('admin.kelas.store') }}" method="POST">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                    <div>
                        ...
                        <input type="text" name="name" id="input_name" required value="{{ old('name') }}"
                    ...
                    <div>
                        ...
                        <select name="grade" id="input_grade" required
                                class="w-full text-xs rounded-xl border-slate-200 dark:border-slate-700
                                       bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-100
                                       focus:border-indigo-500 focus:ring-indigo-500 transition shadow-sm">
                            <option value="" disabled {{ old('grade') ? '' : 'selected' }}>Pilih Tingkat</option>
                            <option value="7" {{ old('grade') == '7' ? 'selected' : '' }}>Tingkat 7 (VII)</option>
                            <option value="8" {{ old('grade') == '8' ? 'selected' : '' }}>Tingkat 8 (VIII)</option>
                            <option value="9" {{ old('grade') == '9' ? 'selected' : '' }}>Tingkat 9 (IX)</option>
                        </select>
                    </div>
                </div>

                {{-- Baris 2: Sub Bagian & Wali Kelas --}}
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                    <div>
                        <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1.5">
                            Sub Bagian / Suffix <span class="text-rose-500">*</span>
                        </label>
                        <input type="text" name="section" id="input_section" required value="{{ old('section', 'A') }}"
                    ...
                    <div>
                        ...
                        <select name="semester" id="input_semester" required
                                class="w-full text-xs rounded-xl border-slate-200 dark:border-slate-700
                                       bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-100
                                       focus:border-indigo-500 focus:ring-indigo-500 transition shadow-sm">
                            <option value="1" {{ old('semester', ($currentMonth >= 7 && $currentMonth <= 12 ? '1' : '2')) == '1' ? 'selected' : '' }}>Semester 1 (Ganjil)</option>
                            <option value="2" {{ old('semester', ($currentMonth >= 7 && $currentMonth <= 12 ? '1' : '2')) == '2' ? 'selected' : '' }}>Semester 2 (Genap)</option>
                        </select>
                    </div>
                </div>
                ...
                {{-- Deskripsi/Catatan Tambahan Ruangan --}}
                <div class="mb-4">
                    <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1.5">
                    ...
                    <textarea name="room" id="input_room" rows="2" placeholder="Contoh: Gedung B Lantai 2, Samping Lab IPA"
                              class="w-full text-xs rounded-xl border-slate-200 dark:border-slate-700
                                     bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-100
                                     focus:border-indigo-500 focus:ring-indigo-500 transition shadow-sm resize-none">{{ old('room') }}</textarea>
                </div>
