    ...
    <div class="bg-white dark:bg-slate-800 w-full max-w-3xl rounded-[2.5rem] shadow-2xl border border-white/20 dark:border-slate-700 max-h-[94vh] flex flex-col overflow-hidden animate-in zoom-in-95 duration-300">
        ...
        <div class="px-8 py-6 border-b border-slate-100 dark:border-slate-700 flex items-center justify-between bg-white/50 dark:bg-slate-800/50 backdrop-blur-md sticky top-0 z-10">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5 font-medium truncate max-w-[400px]">ID: #{{ $pengumuman->id }} — {{ $pengumuman->judul }}</p>
                </div>
            </div>
        ...
        <form method="POST" action="{{ route('admin.pengumuman.update', $pengumuman) }}" 
                ...
                <div class="space-y-6">
                    ...
                    <div id="sectionTeks" class="tipe-section animate-in fade-in slide-in-from-top-2 duration-300">
                        ...
                        <textarea id="isiTeks" name="isi" rows="6" 
                                  class="w-full px-5 py-4 rounded-[1.5rem] border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-sm leading-relaxed transition-all resize-none min-h-[200px]">{{ old('isi', $pengumuman->isi) }}</textarea>
                    </div>
                    ...
                    <div id="sectionGambar" class="tipe-section hidden animate-in fade-in slide-in-from-top-2 duration-300">
                        ...
                        <textarea id="isiGambar" rows="3" class="w-full px-5 py-4 rounded-2xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-sm transition-all resize-none">{{ old('isi', $pengumuman->isi) }}</textarea>
                    </div>
                    ...
                    <div id="sectionDokumen" class="tipe-section hidden animate-in fade-in slide-in-from-top-2 duration-300">
                        ...
                        <div class="mb-6 p-5 bg-indigo-50 dark:bg-indigo-900/30 rounded-2xl border border-indigo-100 dark:border-indigo-800">
                            <div class="flex items-center justify-between">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div class="min-w-0">
                                        <p class="text-xs font-bold text-indigo-700 dark:text-indigo-300 truncate max-w-[200px]">{{ $pengumuman->file_name }}</p>
                                        <a href="{{ asset('storage/' . $pengumuman->file_path) }}" target="_blank" class="text-[10px] text-indigo-500 font-bold hover:underline">LIHAT BERKAS ↗</a>
                                    </div>
                                </div>
                        ...
                        <textarea id="isiDokumen" rows="3" class="w-full px-5 py-4 rounded-2xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-sm transition-all resize-none">{{ old('isi', $pengumuman->isi) }}</textarea>
                    </div>
                    ...
                    <div id="sectionLink" class="tipe-section hidden space-y-6 animate-in fade-in slide-in-from-top-2 duration-300">
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="url" name="link_url" value="{{ old('link_url', $pengumuman->link_url) }}" 
                            ...
                            <div>
                                ...
                                <input type="text" name="link_label" value="{{ old('link_label', $pengumuman->link_label ?? 'Kunjungi Link') }}" 
                                       class="w-full px-5 py-3.5 rounded-2xl border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-700 text-sm font-bold focus:ring-4 focus:ring-indigo-500/10 transition-all">
                            </div>
                        </div>
                        <div>
                            <label class="block text-xs font-bold text-slate-500 dark:text-slate-400 mb-2 uppercase tracking-wider">Keterangan</label>
                            <textarea id="isiLink" rows="3" class="w-full px-5 py-4 rounded-2xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-sm transition-all resize-none">{{ old('isi', $pengumuman->isi) }}</textarea>
                        </div>
                    </div>
                        ...
                        <label class="flex items-center justify-between p-5 bg-slate-50 dark:bg-slate-900/50 rounded-3xl border border-slate-100 dark:border-slate-700 cursor-pointer hover:border-indigo-200 dark:hover:border-indigo-800 transition-all">
                            ...
                            <div class="relative inline-flex items-center cursor-pointer">
                                <input type="checkbox" name="{{ $toggle['name'] }}" value="1" 
                                       id="{{ $toggle['id'] }}"
                                       {{ old($toggle['name'], $pengumuman->{$toggle['name']}) ? 'checked' : '' }}
                                       class="sr-only peer">
            ...
            <div class="px-8 py-6 border-t border-slate-100 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/50 backdrop-blur-md flex items-center justify-between">
                <a href="{{ route('admin.pengumuman') }}" class="text-sm font-bold text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 transition-colors">Batal</a>
                <button type="submit" 
