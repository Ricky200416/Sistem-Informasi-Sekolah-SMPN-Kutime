...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div class="bg-white dark:bg-slate-800 rounded-3xl p-5 border border-slate-200 dark:border-slate-700 shadow-sm flex items-center gap-4">
            <div class="w-12 h-12 {{ $st['bg'] }} dark:bg-slate-700 rounded-2xl flex items-center justify-center text-2xl">
                {{ $st['icon'] }}
            </div>
            <div>
                <div class="text-xl font-bold {{ $st['c'] }}">{{ $st['val'] }}</div>
                <div class="text-xs text-slate-400 dark:text-slate-500 font-medium">{{ $st['label'] }}</div>
            </div>
        </div>
        @endforeach
    </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-slate-700/50" id="pgTableBody">
                    ...
                    @endphp
                    <tr class="hover:bg-indigo-50/30 dark:hover:bg-indigo-900/10 transition-colors group pg-row" data-id="{{ $item->id }}">
                        <td class="px-6 py-4">
                            <div class="flex items-center justify-center">
                                <input type="checkbox"
                            ...
                            <button type="button" onclick='pgBuka({{ $pgJson }})'
                                ...
                                <div class="min-w-0">
                                    ...
                                    <p class="text-[11px] text-slate-400 dark:text-slate-500 mt-0.5 line-clamp-1">
                                        {{ strip_tags($item->isi) ?: 'Tanpa keterangan teks' }}
                                    </p>
                                </div>
                            </button>
                        </td>
                        ...
                        <td class="px-4 py-4">
                            <button type="button"
                                    ...
                                    class="relative inline-flex h-6 w-11 items-center rounded-full
                                           ...
                                           {{ $item->is_active ? 'bg-indigo-500 shadow-lg shadow-indigo-500/30' : 'bg-slate-300 dark:bg-slate-600' }}">
                                <span class="inline-block h-4 w-4 transform rounded-full bg-white shadow-sm transition-transform duration-300"
                                      style="{{ $item->is_active ? 'transform:translateX(24px)' : 'transform:translateX(4px)' }}">
                                </span>
                            </button>
                        </td>
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center justify-end gap-2">
                                <button type="button" onclick='pgBuka({{ $pgJson }})' title="Pratinjau"
