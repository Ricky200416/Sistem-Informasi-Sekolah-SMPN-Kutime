...
<div x-data="galeriManager()" x-cloak class="space-y-4">
    ...
    <div class="bg-white rounded-lg shadow-sm border border-slate-200 p-4">
        ...
        <div class="grid grid-cols-3 gap-2 mb-4">
            ...
            <div class="bg-<?php echo e($s['color']); ?>-50 border border-<?php echo e($s['color']); ?>-200 rounded-lg p-2.5 text-center">
                <p class="text-lg font-bold text-<?php echo e($s['color']); ?>-700"><?php echo e($s['value']); ?></p>
                <p class="text-xs text-<?php echo e($s['color']); ?>-600"><?php echo e($s['label']); ?></p>
            </div>
        ...
        <form method="GET" action="<?php echo e(route('admin.kelola-website')); ?>" class="flex flex-wrap gap-2 mb-4">
            ...
            <select name="galeri_kategori"
                ...
                <?php $__currentLoopData = $galeriKategoriOptions; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $val => $label): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <option value="<?php echo e($val); ?>" <?php if(request('galeri_kategori') === $val): echo 'selected'; endif; ?>><?php echo e($label); ?></option>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            </select>
            ...
            <select name="galeri_tipe"
                ...
                <option value="photo"         <?php if(request('galeri_tipe') === 'photo'): echo 'selected'; endif; ?>>🖼️ Foto</option>
                <option value="video"         <?php if(request('galeri_tipe') === 'video'): echo 'selected'; endif; ?>>🎥 Video</option>
                <option value="link_youtube"  <?php if(request('galeri_tipe') === 'link_youtube'): echo 'selected'; endif; ?>>▶️ YouTube</option>
                <option value="link_facebook" <?php if(request('galeri_tipe') === 'link_facebook'): echo 'selected'; endif; ?>>📘 Facebook</option>
            </select>

            <select name="galeri_status"
                    class="rounded-md border-slate-300 text-xs py-1.5 focus:border-indigo-500 focus:ring-indigo-500">
                <option value="">Semua Status</option>
                <option value="aktif" <?php if(request('galeri_status') === 'aktif'): echo 'selected'; endif; ?>>Aktif</option>
                <option value="draf"  <?php if(request('galeri_status') === 'draf'): echo 'selected'; endif; ?>>Draf</option>
            </select>

            <button type="submit"
                    class="px-3 py-1.5 bg-slate-700 text-white text-xs rounded-lg hover:bg-slate-800 transition">Filter</button>

            <?php if(request()->hasAny(['galeri_kategori','galeri_tipe','galeri_status'])): ?>
        ...
        <?php else: ?>
            <div class="grid sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
                ...
                <div class="group relative bg-slate-100 rounded-lg overflow-hidden aspect-[4/3] border border-slate-200 <?php echo e($item->status === 'draf' ? 'opacity-60' : ''); ?>">

                    
                    <img src="<?php echo e($item->thumbnail_url); ?>" alt="<?php echo e($item->judul); ?>"
                    ...
                    <div class="absolute inset-0 bg-black/55 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col justify-between p-2">
                        <p class="text-white text-xs font-medium line-clamp-2 leading-tight"><?php echo e($item->judul); ?></p>
                            ...
                            <div class="flex gap-1.5">
                                ...
                                <form action="<?php echo e(route('admin.galeri.toggle-status', $item)); ?>" method="POST" class="inline">
                                    ...
                                    <button type="submit" class="p-1 bg-white/20 hover:bg-white/30 rounded text-white transition"
                                            title="<?php echo e($item->status === 'aktif' ? 'Jadikan Draf' : 'Aktifkan'); ?>">
                                ...
                                <form action="<?php echo e(route('admin.galeri.destroy', $item)); ?>" method="POST" class="inline"
            ...
            <?php if($galeris->hasPages()): ?>
                <div class="mt-3 text-xs"><?php echo e($galeris->links()); ?></div>
            <?php endif; ?>
        <?php endif; ?>
    ...
    <div class="bg-green-50 border border-green-200 rounded-lg p-3 text-xs">
        ...
        <a href="<?php echo e(route('website.galeri')); ?>" target="_blank" class="text-indigo-600 hover:underline flex items-center gap-1">
...
</script><?php /**PATH C:\PROJECT\smpn-kutime\resources\views/admin/kelola-website/tabs/galeri.blade.php ENDPATH**/ ?>