...
<?php
    /*
     * FIX: Hapus filter tanggal_selesai — tampilkan SEMUA pengumuman aktif
     * untuk guru/semua, tanpa memblokir yang sudah melewati tanggal_selesai.
     * Pengumuman kadaluarsa tetap tampil dengan badge "Sudah Berakhir".
     */
    if (!isset($pengumuman)) {
        ...
        <div class="relative px-7 py-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <div class="flex items-center gap-3 flex-shrink-0">
                <div class="rounded-2xl px-4 py-2.5 text-center border"
                    ...
                    <p class="text-2xl font-black text-white leading-none"><?php echo e($pengumuman->total()); ?></p>
                    <p class="text-violet-200 text-xs font-medium mt-0.5">Pengumuman</p>
                </div>
                <div class="rounded-2xl px-4 py-2.5 text-center border"
                     style="background:rgba(255,255,255,0.15);border-color:rgba(255,255,255,0.2)">
                    <p class="text-sm font-bold text-white leading-snug"><?php echo e(now()->isoFormat('D MMM')); ?></p>
                    <p class="text-violet-200 text-xs font-medium mt-0.5"><?php echo e(now()->isoFormat('Y')); ?></p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="w-full bg-white dark:bg-slate-800
        ...
        <div class="px-6 py-4 border-b border-slate-100 dark:border-slate-700/70
            ...
            <div class="flex items-center gap-2.5">
                ...
                <p class="text-sm font-semibold text-slate-600 dark:text-slate-300">
                    <?php echo e($pengumuman->firstItem()); ?>–<?php echo e($pengumuman->lastItem()); ?>

                    <span class="font-normal text-slate-400">dari</span>
                    <?php echo e($pengumuman->total()); ?> pengumuman
                </p>
            </div>
        ...
        <ul class="divide-y divide-slate-100 dark:divide-slate-700/60">
            ...
            <?php
                ...
                // Baru jika dibuat dalam 24 jam terakhir
                $isNew = $item->created_at->gt(now()->subHours(24));

                // Segera berakhir: punya tanggal_selesai, belum lewat, ≤ 3 hari lagi
                $segBerakhir = $item->tanggal_selesai
                    && !$item->tanggal_selesai->isPast()
                    && $item->tanggal_selesai->diffInDays(now()) <= 3;

                // Sudah kadaluarsa: punya tanggal_selesai dan sudah lewat
                $isExpired = $item->tanggal_selesai && $item->tanggal_selesai->isPast();
            ?>

            <li>
                <button type="button"
                        onclick='pgBuka(<?php echo e($pgJson); ?>)'
                        class="w-full text-left group px-6 py-5
                               ...
                               <?php echo e($isExpired ? 'opacity-60' : ''); ?>">

                    <div class="flex items-start gap-4">

                        <div class="flex flex-col items-center shrink-0">
                            <div class="w-10 h-10 rounded-2xl <?php echo e($aksenBg); ?> border
                                        flex items-center justify-center shrink-0 shadow-sm">
                                <span class="text-lg leading-none"><?php echo e($item->tipeIcon()); ?></span>
                            </div>
                            <?php if(!$loop->last): ?>
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <div class="flex items-start justify-between gap-3 mb-1.5">
                                <div class="flex items-center gap-2 flex-1 min-w-0 flex-wrap">
                                    <h3 class="font-bold text-slate-800 dark:text-slate-100 text-[15px] leading-snug
                                               ...
                                               transition-colors <?php echo e($isExpired ? 'line-through decoration-slate-400' : ''); ?>">
                            ...
                            <?php elseif($item->tipe_konten === 'gambar'): ?>
                            ...
                            <?php elseif($item->tipe_konten === 'dokumen'): ?>
                                <div class="flex items-center gap-2.5 w-fit px-3 py-2 rounded-xl
                                            <?php echo e($aksenBg); ?> border">
                                    <svg class="w-4 h-4 <?php echo e($aksenTeks); ?> shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                                    </svg>
                                    <div class="min-w-0">
                                        <p class="text-[11px] font-bold <?php echo e($aksenTeks); ?> uppercase leading-none">
                                            <?php echo e($item->fileExtension() ?: 'DOKUMEN'); ?>
                            ...
                            <?php elseif($item->tipe_konten === 'link'): ?>
                                <div class="flex items-center gap-2 w-fit px-3 py-2 rounded-xl
                                            <?php echo e($aksenBg); ?> border">
                                    <svg class="w-4 h-4 <?php echo e($aksenTeks); ?> shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1"/>
                                    </svg>
                                    <span class="text-xs <?php echo e($aksenTeks); ?> font-semibold truncate max-w-[240px]">
                            ...
                            <?php if($item->tanggal_selesai && !$segBerakhir && !$isExpired): ?>
                            <div class="mt-2.5 inline-flex items-center gap-1.5 text-[11px] font-medium
                                ...
                                Berakhir <?php echo e($item->tanggal_selesai->isoFormat('D MMM Y, HH:mm')); ?>
        ...
        <div class="px-6 py-4 border-t border-slate-100 dark:border-slate-700/70
            ...
            <p class="text-xs text-slate-400 dark:text-slate-500 order-last sm:order-first">
                Halaman <strong class="text-slate-600 dark:text-slate-300"><?php echo e($pengumuman->currentPage()); ?></strong>
                dari <strong class="text-slate-600 dark:text-slate-300"><?php echo e($pengumuman->lastPage()); ?></strong>
            </p>

            <div class="flex items-center gap-1.5">

                <?php if($pengumuman->onFirstPage()): ?>
                ...
                <?php else: ?>
                <a href="<?php echo e($pengumuman->previousPageUrl()); ?>"
                ...
                <?php if($start > 1): ?>
                <a href="<?php echo e($pengumuman->url(1)); ?>"
                ...
                <?php if($end < $pengumuman->lastPage()): ?>
                <?php if($end < $pengumuman->lastPage() - 1): ?>
                <span class="text-slate-300 dark:text-slate-600 text-xs px-1">…</span>
                <?php endif; ?>
                <a href="<?php echo e($pengumuman->url($pengumuman->lastPage())); ?>"
                   class="w-8 h-8 flex items-center justify-center rounded-xl text-xs font-bold
                          text-slate-500 hover:bg-slate-100 dark:text-slate-400 dark:hover:bg-slate-700 transition-colors">
                    <?php echo e($pengumuman->lastPage()); ?>

                </a>
                <?php endif; ?>

                <?php if($pengumuman->hasMorePages()): ?>
                <a href="<?php echo e($pengumuman->nextPageUrl()); ?>"
                   class="inline-flex items-center gap-1 px-3.5 py-2 rounded-xl text-xs font-semibold
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\PROJECT\smpn-kutime\resources\views/guru/pengumuman/index.blade.php ENDPATH**/ ?>